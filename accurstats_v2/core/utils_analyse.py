...
interpolations= ['Linéaire', 'Par valeur suivante', 'Splines cubiques', 'Kalman', 'Médiane', 'Moyenne']
...
def interpolate(data: DataFrame, i: int, inter:str) ->DataFrame:
    if inter==interpolations[0]:
        return linear_interpol(data,i)
    if inter == interpolations[1]:
        return next_interpol(data,i)
    if inter == interpolations[2]:
        return previous_interpol(data,i)
    if inter == interpolations[3]:
        return spline_interpol(data,i)
    if inter == interpolations[4]:
        return kalman_interpol(data, i)
    if inter== interpolations[5]:
        return mean_interpol(data,i)
    if inter == interpolations[6]:
        return mean_interpol(data,i)
